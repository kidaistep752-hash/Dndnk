-- ====================================================================
--  BLOCK 1: SERVICES, CONFIG & DESIGN CARCASS
-- ====================================================================
...
-- Жесткая очистка памяти от старых зависших копий
...
-- Плавающая закругленная кнопка-шестеренка
...
-- ОТДЕЛЬНОЕ ЗАКРУГЛЕННОЕ ОКНО ДЛЯ RADAR MM2
local MM2Window = Instance.new("Frame")
MM2Window.Size = UDim2.new(0, 180, 0, 150)
MM2Window.Position = UDim2.new(0.5, 215, 0.5, -75) -- Справа от основного меню
...
MM2Stroke.Color = Color3.fromRGB(255, 65, 65) -- Фирменный красный неон
...
-- Привязка переключения видимости главного фрейма
OpenButton.MouseButton1Click:Connect(function()
    MainFrame.Visible = not MainFrame.Visible
end)
-- ====================================================================
--  PART 2.1: TAB MANAGER & AUTO-SLIDING SLIDERS
-- ====================================================================
local pages = {}
...
local function createFeatureWithLeftSlider(parent, configKey, speedKey, text, min, max)
    ...
    BigWrapper.AutomaticSize = Enum.AutomaticSize.Y
...
-- ====================================================================
--  PART 2.2: ADAPTIVE TARGET & GOD MODE CONTAINER (PLAYER TAB)
-- ====================================================================

-- 1. Раздвижной ТАРГЕТ во вкладке PLAYER
...
-- 2. Раздвижной GOD MODE во вкладке PLAYER
...
-- ====================================================================
--  PART 2.3: VISUAL PALETTE, GAME MM2 & SERVER MATCHMAKING
-- ====================================================================
...
-- 2. Наполнение GAME TAB (MM2 ТРИГГЕР ОКНА И АВТО-ОБНОВЛЕНИЯ)
...
MM2Btn.MouseButton1Click:Connect(function()
    ...
    -- Синхронное открытие окна при включении мода
...
-- ====================================================================
--  PART 3: ENGINE LOOPS, XNEO FLY, PUSH FLING & ENEMY HIGHLIGHT ESP
-- ====================================================================
...
RunService.RenderStepped:Connect(function()
    ...
    -- СИНХРОНИЗАЦИЯ ОКНА MM2 С ГЛАВНЫМ МЕНЮ
    ...
    -- ПОЛЁТ XNEO ЧЕРЕЗ TRANSLATEBY (ПЕРСОНАЖ СТОИТ НА МЕСТЕ, ЕСЛИ НЕ ЖАТЬ ХОД)
    ...
    -- ИСПРАВЛЕННЫЙ ТВОЙ МЕТОД ONE PUNCH FLING (ВРАГ УЛЕТАЕТ ВПЕРЕД ОТ ТВОЕГО ВЗГЛЯДА)
    ...
    -- HIGHLIGHT ESP (ВРАГИ ПОДСВЕЧИВАЮТСЯ, ТЫ — НЕТ)
    for _, p in pairs(Players:GetPlayers()) do
